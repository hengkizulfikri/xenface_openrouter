...
function Camera() {
  ...
  const isPortrait = usePotrait(); // 9/16 => 16/9 (untuk responsive dari camera nya)
  ...
  function capturePhoto() {
    setErrorMessage("")
    const video = webcamRef.current?.video; // => funsi tanya tersebut jika datanya tidak ada maka dia akan jadi undefined
    const canvas = canvasRef.current;

    if(!video || !canvas || !video.videoWidth) {
      setErrorMessage("Kamera Belum Menyala, coba tunggu bentar fren!");
    }
        ...
        <div className="bg-gray-800 p-6 mt-8 rounded-xl shadow border border-gray-700">
          ...
          {htmlToRender.trim() ? (
            ...
            <div className='bg-gray-500 p-4'>
              <p className='font-semibold text-white'>Ambil Foto Kamu Lalu Tekan Ramalkan! 
                Agar Kamu tau Kondisi kamu Terkini</p>
            </div> 
            )}
        </div>
      </section>
